...
class ReferenceVariantsHelper(
    ...
    private fun getReferenceVariantsNoVisibilityFilter(
        ...
        val languageVersionSettings = resolutionFacade.frontendService<org.jetbrains.kotlin.config.LanguageVersionSettings>()
        val dataFlowValueFactory = resolutionFacade.frontendService<org.jetbrains.kotlin.resolve.calls.smartcasts.DataFlowValueFactory>()
    ...
    private fun MutableSet<DeclarationDescriptor>.addScopeAndSyntheticExtensions(
        ...
        for (descriptor in scope.collectDescriptorsFiltered(kindFilter exclude DescriptorKindExclude.NonExtensions, nameFilter, true)) {
        ...
        if (kindFilter.acceptsKinds(DescriptorKindFilter.VARIABLES_MASK)) {
            for (extension in syntheticScopes.collectSyntheticExtensionProperties(receiverTypes, NoLookupLocation.FROM_IDE)) {
                process(extension)
            }
        }
